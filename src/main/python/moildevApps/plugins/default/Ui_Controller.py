...
from .Ui_Utils import read_image
...
class Controller(QtWidgets.QMainWindow):
    ...
    def __init__(self, MainWindow):
        ...
        self.videoControl = Video_Controler(self, self.ctx)
    ...
    def open_image(self):
        """Load image frame
        """
        file_image, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select Image', "", "Image Files (*.jpeg *.jpg "
                                                                                        "*.png *.gif *.bmg)")
        if file_image:
            file_name, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select Parameter", "",
            ...
            if file_name:
                ...
                self.cam = False
                self.moildev = Moutils(file_name)
                self.anypoint.resetAlphaBeta()

    def open_video_file(self):
        """Load video file.
        """
        videoFile, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select Video Files",
                                                             "Image Files (*.mp4 *.avi *.mpg *.gif *.mov)")
        if videoFile:
            paramName, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select Parameter", "Parameter Files (*.json)")
            if paramName:
                self.anypoint.resetAlphaBeta()
                self.videoControl.videoButtonEnable()
                self.coordinate_point = None
                self.moildev = Moutils(paramName)
    ...
    def cameraOpen(self):
        ...
        if self.videoStreamURL is None:
            ...
            if self.image is None:
                ...
                file_name, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select Left Parameter",
                                                                     "Parameter Files (*.json)")
                if file_name:
                    self.moildev = Moutils(file_name)
        ...
        if self.image is None:
            ...
            if e.button() == QtCore.Qt.LeftButton:
                ...
                if self.ui.btn_Anypoint.isChecked():
                    self.alpha, self.beta = self.moildev.get_alpha_beta(delta_x, delta_y, self.anypointState)
                    self.anypoint.anypoint_view()
                elif self.ui.checkBox_ShowRecenterImage.isChecked():
                    self.alpha, self.beta = self.moildev.get_alpha_beta(delta_x, delta_y, 1)
