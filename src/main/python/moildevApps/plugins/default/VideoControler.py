...
class Video_Controler(object):
    ...
    :type : -
    """
    def __init__(self, Mainwindow, ctx):
        """Constructor method
        """
    ...
    def videoPlayPouse(self):
        ...
        if self.play:
            self.timer.stop()
            self.parent.ui.videoPlay.setIcon(QtGui.QIcon(QtGui.QPixmap.fromImage(self.ctx.ctx.icon_play)))
            self.pause_video()
            self.play = False

        else:
            self.parent.ui.videoPlay.setIcon(QtGui.QIcon(QtGui.QPixmap.fromImage(self.ctx.ctx.icon_pause)))
            self.play_video()
            self.play = True
